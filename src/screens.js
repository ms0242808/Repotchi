...
const { NUTRITION } = require('./config');
...
function canvasFor(opts) {
	...
	return new Canvas({
		width,
		p: opts.palette || a.palette(false),
...
function onboarding(opts = {}) {
	...
	c.row(`     ${p.dim('your pet eats it within a second')}`);
	c.row();
	...
	c.sep();
	c.row(
		` ${p.gold('[t]')}${p.dim('rack here')}  ${p.gold('[?]')}${p.dim('help')}  ${p.gold('[q]')}${p.dim('uit')}   ${p.dim(`${arrow} setup trouble? run`)} ${p.snow('pet doctor')}`
	);
	c.bottom();
	return c.lines;
}
...
function stats(state, opts = {}) {
	...
	c.row(
		`  ${p.dim(a.pad('hatched', 14))}${p.snow(new Date(state.hatchedAt).toLocaleString())} ${p.dim(`(${humanAge(now - state.hatchedAt)} ago)`)}`
	);
	c.row(
		`  ${p.dim(a.pad('last meal', 14))}${state.lastEventAt ? p.snow(`${humanAge(now - state.lastEventAt)} ago`) : p.gold('never')}`
	);
	...
	for (const [label, n, colour] of meals) {
		...
		c.row(
			`  ${p.dim(a.pad(label, 16))}${p[colour]((ascii ? '#' : '\u2588').repeat(filled))}${p.gray((ascii ? '.' : '\u2591').repeat(barWidth - filled))} ${a.padStart(String(n), 5)}`
		);
	}
	c.row();
	...
	for (const stage of evo.road) {
		...
		const name = stage.here
			? p.bold(p.snow(a.pad(stage.name, 16)))
			: stage.reached
				? p.snow(a.pad(stage.name, 16))
				: p.gray(a.pad(stage.name, 16));
		const at = p.dim(`lv ${String(stage.min).padStart(2)}  ${String(stage.xpAt).padStart(5)} xp`);
		let tail = '';
		if (stage.here)
			tail = p.gold(`  ${ascii ? '<-' : '\u2190'} you, ${snap.xpInto}/${snap.xpNeed} into lv ${snap.level}`);
		else if (evo.next && stage.key === evo.next.key) {
			const eta = evo.etaDays
				? ` ${ascii ? '~' : '\u2248'}${evo.etaDays} days at ${evo.xpPerDay} xp/day`
				: ' (no recent pace to estimate)';
	...
	c.row(
		` ${p.gold('[i]')}${p.dim(' or ')}${p.gold('[esc]')}${p.dim(' back')}   ${p.gold(`[${arrows}]`)}${p.dim(' [j][k] [PgUp][PgDn] wheel: scroll')}   ${p.gold('[q]')}${p.dim('uit')}`
	);
	c.bottom();
	return c.lines;
}
...
function help(opts = {}) {
	...
	c.sep('scrolling');
	c.row(`  ${p.dim('When a screen is taller than the terminal:')}`);
	c.row(
		`  ${p.gold(a.pad(ascii ? 'up/dn' : '\u2191 \u2193', 12))}${p.snow('one line')}  ${p.dim('(also')} ${p.gold('k')} ${p.dim('/')} ${p.gold('j')}${p.dim(')')}`
	);
	c.row(`  ${p.gold(a.pad('PgUp PgDn', 12))}${p.snow('one page')}  ${p.dim('(also space)')}`);
	c.row(`  ${p.gold(a.pad('g G', 12))}${p.snow('top / bottom')}  ${p.dim('(also Home / End)')}`);
	c.row(`  ${p.gold(a.pad('wheel', 12))}${p.snow('three lines per notch')}`);
	c.row();
	...
	c.sep();
	c.row(` ${p.dim('any other key goes back')}`);
	c.bottom();
	return c.lines;
}
